...
const getGarages = async (coordinates) => {
  try {
    const { data } = await axios.get(`${import.meta.env.VITE_RESERVATION_HOST}/reservations`, {
      params: coordinates,
    });
    return data;
  } catch (err) {
...
const getReservationsByDate = async (garage_id, date) => {
  ...
  try {
    const { data } = await axios.get(
      `${import.meta.env.VITE_RESERVATION_HOST}/reservations/garage/${garage_id}?date=${date}`
    );
    return data;
  } catch (error) {
    console.log(error);
...
const postReservation = async (body) => {
  /* Given garage_id and date, get count of reservations per hour */
  try {
    await axios.post(`${import.meta.env.VITE_RESERVATION_HOST}/reservations`, body);
